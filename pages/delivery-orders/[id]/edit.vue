<template>
	<div class="punch-page-wrapper bg-white rounded-[24px] p-6">
		...
		<div v-if="detailData" class="tabs pt-8">
			<ForwarderInfomationEditTab
				v-if="currentActiveTab === 1"
				:forwarder="detailData"
...
<script lang="ts">
	...
	import ForwarderInfomationEditTab from '../../../components/Tabs/ForwarderInfomationEditTab.vue';
	export default {
		components: { ForwarderInfomationEditTab },
		setup() {
			...
			useHead({
				...
				meta: [{ name: 'description', content: 'Chỉnh sửa đối tác vận chuyển' }],
			});
			return {
				api,
				listTabs,
		...
		mounted() {
			...
			this.setCurrentViewTitle('Chỉnh sửa đối tác vận chuyển');
			this.getDetailData();
		},
		methods: {
			async getDetailData() {
