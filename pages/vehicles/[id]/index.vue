<template>
	<div class="punch-page-wrapper bg-white rounded-[24px] p-6">
		...
		<div v-if="detailData" class="tabs pt-8">
			<ForwarderInfomationTab v-if="currentActiveTab === 1" :forwarder="detailData" @on-edit="onEdit" />
		</div>
	</div>
</template>
<script lang="ts">
	...
	import ForwarderInfomationTab from '../../../components/Tabs/ForwarderInfomationTab.vue';
	export default {
		components: { ForwarderInfomationTab },
		setup() {
			const { setBreadCrumbWithHomePage, setCurrentViewTitle } = useAppStateStore();
			const { $api } = useNuxtApp();
			const api = $api;
			const listTabs = reactive([
				{ id: 1, name: 'Thông tin nhà vận chuyển' },
			...
			useHead({
				...
				meta: [{ name: 'description', content: 'Chi tiết đối tác vận chuyển' }],
			});
			return {
				api,
				listTabs,
		...
		data() {
			return {
				detailData: null as Forwarder | null,
				loading: false,
				result: 0,
			};
		},
		mounted() {
			...
			this.setCurrentViewTitle('Chi tiết đối tác');
			this.getDetailData();
		},
		methods: {
			async getDetailData() {
				await this.api.forwarder
					.getDetailData(this.$route.params.id as string, {})
					.then(
						res => {
							this.detailData = res.data.forwarder as Forwarder;
						},
						err => {
							console.log(err);
						}
