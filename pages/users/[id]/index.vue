<template>
	<div class="punch-page-wrapper bg-white rounded-[24px] p-6">
		<NuxtLink to="/users" class="w-10 h-10 flex items-center justify-center rounded-half hover:bg-gray-400/30">
		...
		<div v-if="detailData" class="tabs pt-8">
			<UserInfomationTab v-if="currentActiveTab === 1" :detail-data="detailData" @on-edit="onEdit" />
		</div>
...
<script lang="ts">
	...
	import { AccountUser } from '../../../models/AccountUsers';
	import NormalButton from '../../../components/Buttons/NormalButton.vue';
	import UserInfomationTab from '../../../components/Tabs/UserInfomationTab.vue';
	export default {
		components: { UserInfomationTab, NormalButton },
		setup() {
			...
			useHead({
				...
				meta: [{ name: 'description', content: 'Thông tin nhân viên' }],
			});
			return {
				api,
				listTabs,
		...
		data() {
			return {
				detailData: null as AccountUser | null,
				loading: false,
				result: 0,
			};
		},
		mounted() {
			...
			this.setCurrentViewTitle('Thông tin nhân viên');
			this.getDetailData();
		},
		methods: {
			async getDetailData() {
				await this.api.accountUsers
					...
					.then(
						res => {
							this.detailData = res.data.user as AccountUser;
						},
						err => {
							console.log(err);
						}
