							...
							<table class="min-w-full rounded-lg divide-y divide-gray-100 divide-y-4">
								<thead class="bg-neutral-100 sticky top-0 z-[4] border-b-1 text-neutral-300">
									<tr class="text-left">
										<th
											scope="col"
											class="py-5 pl-4 pr-3 text-left body-md font-normal text-neutral-900 sm:pl-6"
										...
										<th
											scope="col"
											class="px-3 py-5 text-left truncate body-md font-normal text-neutral-900"
											style="max-width: 240px; width: 240px"
										>
											Thời gian đến cửa khẩu
										</th>
								...
								<tbody class="divide-y divide-gray-100 divide-y-4">
									<tr v-for="(item, idx) in listData" :key="item.id" class="table-hover-row text-left">
										...
										<td style="max-width: 120px; width: 120px">
											<div class="whitespace-nowrap px-3 py-4 body-md text-neutral-900 truncate">
												<p>{{ formatDateHM(item.recorded_time) }}</p>
												<p class="text-neutral-400">{{ formatDateDMY(item.recorded_time) }}</p>
											</div>
										</td>

										<td style="max-width: 200px; width: 200px">
											<div class="whitespace-nowrap px-3 py-4 body-md text-neutral-900 truncate body-md">
												<p>{{ formatDateHM(item.arrival_time_at_border_gate) }}</p>
												<p class="text-neutral-400">{{ formatDateDMY(item.arrival_time_at_border_gate) }}</p>
											</div>
										</td>

										<td style="max-width: 120px; width: 120px">
											<div class="whitespace-nowrap px-3 py-4 body-md text-neutral-900">
												{{ item.merchandise_position }}
											</div>
										</td>

										<td>
											<div class="whitespace-nowrap px-3 py-4 body-md text-neutral-900">
												{{ item.temperature_set }}
											</div>
										</td>
										<td>
											<div class="whitespace-nowrap px-3 py-4 body-md text-neutral-900">
												{{ item.temperature_supply }}
											</div>
										</td>
										<td>
											<div class="whitespace-nowrap px-3 py-4 body-md text-neutral-900">
												{{ item.temperature_return }}
											</div>
										</td>
										<td>
											<div class="whitespace-nowrap px-3 py-4 body-md text-neutral-900">
															...
															<div class="px-1 py-1">
																<MenuItem v-slot="{ active }">
																	<button
																		...
																		@click="$emit('select-data-to-edit', item)"
																	>
																		Chỉnh sửa
																	</button>
																</MenuItem>
...
<script lang="ts">
	...
	export default {
		...
		methods: {
			formatDateDMY,
			formatDateHM,
			toggleStatusData(item: DeliveryLog) {
				this.$emit('toggle-data', item);
			},
			getCustomerStatusTextColor,
			onEdit(item: DeliveryLog) {
