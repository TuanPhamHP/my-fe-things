							...
							<table class="min-w-full divide-y divide-gray-300 rounded-lg">
								...
								<tbody class="divide-y divide-gray-200 bg-white">
									<tr
										...
										:class="idx % 2 ? 'bg-gray-100/50' : 'bg-white'"
									>
										<td
											class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-blue-500 sm:pl-6 underline cursor-pointer"
										...
										<td
											...
											:class="[getAccountUserStatusTextColor(item.is_active)]"
...
<script lang="ts">
	...
	import { getAccountUserStatusTextColor, formatDateDMY } from '../../helpers/formatter';
	export default {
		components: {
			Menu,
			MenuButton,
		...
		props: {
			listData: {
				type: Array as PropType<AccountUser[]>,
				default: [],
			},
			loadingFirstData: {
				type: Boolean,
		...
		methods: {
			formatDateDMY,
			toggleStatusData(item: AccountUser) {
				this.$emit('toggle-data', item);
			},
			getAccountUserStatusTextColor,
			viewDetail(item: AccountUser) {
