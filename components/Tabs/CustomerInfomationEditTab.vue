...
<script lang="ts">
	...
	export default {
		...
		methods: {
			...
			syncData() {
				if (this.detailData) {
					this.representative_name = this.detailData.representative_name || '';
			...
			updateData() {
				...
				this.api.customers
					...
					.then(
						res => {
							this.notificationStore.setSnack({ text: 'Cập nhật khách hàng thành công!', type: 'success' });
							this.$router.push(`/customers/${this.$route.params.id}`);
						},
						err => {
							this.notificationStore.setSnack({
								text: err?.response?._data?.message || 'Cập nhật khách hàng thất bại',
								type: 'error',
							});
						}
					)
			...
			async deactiveData(id: number | string) {
				...
				await this.api.customers.updateStatus(id, { status_id: 2 }).then(
					res => {
						this.getDetailData();
			...
			async activeData(id: number | string) {
				this.loadingUpdate = true;
				await this.api.customers.updateStatus(id, { status_id: 1 }).then(
					res => {
						this.getDetailData();
						this.notificationStore.setSnack({ text: 'Thao tác thành công', type: 'success' });
						this.$nextTick(() => {
