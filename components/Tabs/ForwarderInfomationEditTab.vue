	...
	<div>
		...
		<div class="grid grid-cols-2 gap-4">
			...
			<div class="col-span-1">
				<BlockInputVue
					v-model="code"
					title="Mã đối tác vận chuyển"
					placeholder="Mã đối tác vận chuyển"
					container-class="mb-0"
					size="xxl"
					isDisabled
				/>
...
<script lang="ts">
	...
	export default {
		...
		methods: {
			...
			syncData() {
				if (this.forwarder) {
					this.representative_name = this.forwarder.representative_name || '';
					this.username = this.forwarder.root_account?.username || '';
					this.id_no = this.forwarder.id_no || '';
					this.phone = this.forwarder.phone || '';
					this.email = this.forwarder.email || '';
					this.code = this.forwarder.code || '';
					this.address_line = this.forwarder.address?.address_line || '';
					this.province = this.forwarder.address?.province?.code || '';
					if (this.forwarder.address?.province?.code) {
						this.syncAddress();
					}
				}
			},
			...
			getListDistrict(syncDistrict = false) {
				this.api.internal.getListDistrict({ province_code: this.province, pagination: false }).then(
					res => {
						...
						if (syncDistrict) {
							this.district = this.forwarder.address?.district?.code || null;
			...
			getListWard(syncWard = false) {
				this.api.internal.getListWards({ district_code: this.district, pagination: false }).then(
					res => {
						...
						if (syncWard) {
							this.ward_code = this.forwarder.address?.ward?.code || null;
						}
					},
					err => {
						this.notificationStore.setSnack({
			...
			updateData() {
				...
				this.api.forwarder
					.update(this.forwarder.id, payload)
					.then(
						res => {
							this.notificationStore.setSnack({ text: 'Cập nhật đối tác vận chuyển thành công!', type: 'success' });
							this.$router.push(`/forwarders/${this.$route.params.id}`);
						},
						err => {
							this.notificationStore.setSnack({
								text: err?.response?._data?.message || 'Cập nhật đối tác vận chuyển thất bại',
								type: 'error',
							});
						}
					)
			...
			toggleStatusData() {
				...
				if (this.forwarder.status?.id === 1) {
					this.deactiveData(this.forwarder.id);
				} else {
					this.activeData(this.forwarder.id);
				}
			},
			async deactiveData(id: number | string) {
				this.loadingUpdate = true;
				await this.api.forwarder.updateStatus(id, { status_id: 2 }).then(
					res => {
						this.getDetailData();
			...
			async activeData(id: number | string) {
				this.loadingUpdate = true;
				await this.api.forwarder.updateStatus(id, { status_id: 1 }).then(
					res => {
						this.getDetailData();
						this.notificationStore.setSnack({ text: 'Thao tác thành công', type: 'success' });
						this.$nextTick(() => {
