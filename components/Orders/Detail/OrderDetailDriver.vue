<template>
	...
	<div class="pt-6 pb-4 px-0 grid grid-cols-3 gap-x-4 gap-y-4">
		<div class="col-span-1">
			<p class="text-md text-neutral-500 mb-2">Tên lái xe</p>
		...
		<div class="col-span-1">
			...
			<p class="text-md font-semibold text-gray-900 py-2">
				{{ detailData.driver?.phone || '---' }}
			</p>
		</div>
		<div class="col-span-1">
			<p class="text-md text-neutral-500 mb-2">Số CCCD/CMT</p>
			<p class="text-md font-semibold text-gray-900 py-2">
				{{ detailData.driver?.id_no || '---' }}
			</p>
		</div>
		<FileListViewModal
			...
			:title="`Tài liệu thông tin lái xe`"
		...
		computed: {
			listDocs(): FileByGroup[] {
				return [
					{
						group_name: 'CCCD/CMT',
						items: [...(this.detailData.driver?.id_card_files || [])],
					},
					{
						group_name: 'Giấy phép lái xe',
						items: [...(this.detailData.driver?.driver_license_files || [])],
					},
					{
						group_name: 'Giấy thông hành',
						items: [...(this.detailData.driver?.passport_files || [])],
					},
					{
						group_name: 'Dấu khống công ty vận tải',
						items: [...(this.detailData.driver?.company_seal_files || [])],
					},
					{
						group_name: 'Giấy phép kinh doanh vận tải',
						items: [...(this.detailData.driver?.transport_business_license_files || [])],
					},
				] as FileByGroup[];
			},
			visibleRemoveFile() {
