	...
	<div>
		<div class="punch-page-wrapper bg-white rounded-[24px] p-0 flex flex-col">
			<div class="px-6 pb-4">
				...
				<OrderUpdateGeneraInfomation
					:detail-data="detailData"
					:create-flag="createFlag"
					@invalid-data="invalidData"
					@submit-data="collectData"
				/>
				<OrderUpdateCustomerInfomation :detail-data="detailData" />
				<OrderUpdateDriverInfomation
					:detail-data="detailData"
					:create-flag="createFlag"
					@invalid-data="invalidData"
					@submit-data="collectData"
				/>
				<OrderUpdateVehicleInfomation
					:detail-data="detailData"
					:create-flag="createFlag"
					@invalid-data="invalidData"
					@submit-data="collectData"
		...
		<div class="btn-block fixed bg-white flex items-center justify-center gap-4">
			<NormalButton
				v-if="detailData.status.id === 10"
				size="xxl-md"
				type="submit"
			...
			>
				{{ detailData.status.id === 10 ? 'Lưu nháp' : 'Lưu nháp' }}
			</NormalButton>
			<NormalButton
				size="xxl-md"
				type="button"
			...
			>
				{{ detailData.status.id === 10 ? 'Lưu & Bắt đầu' : 'Lưu' }}
			</NormalButton>
		</div>
	</div>
</template>
...
<script lang="ts">
	...
	import OrderUpdateGeneraInfomation from './OrderUpdateGeneraInfomation.vue';
	import OrderUpdateCustomerInfomation from './OrderUpdateCustomerInfomation.vue';
	import OrderUpdateDriverInfomation from './OrderUpdateDriverInfomation.vue';
	import OrderUpdateVehicleInfomation from './OrderUpdateVehicleInfomation.vue';

	import { OrderUpdateForm, OrderDetail } from '../../../models/Order';
	export default {
		props: {
			detailData: {
		...
		data() {
			return {
				createFlag: 1,
				invalidTab: ['generalInfo', 'vehicle', 'driver'] as string[],
				formData: {} as OrderUpdateForm,
				refSubmit: 1,
				loadingCreate: false,
				loadingDraft: false,
				isDraft: false,
		...
		methods: {
			...
			invalidData(data: 'generalInfo' | 'vehicle' | 'driver') {
			...
			submitData() {
				...
				this.api.orders
					...
					.then(
						res => {
							this.notificationStore.setSnack({ text: 'Cập nhật đơn hàng thành công!', type: 'success' });
							this.$router.push(`/orders/${res.data.order.id}`);
						},
						err => {
							this.notificationStore.setSnack({
								text: err?.response?._data?.message || 'Cập nhật đơn hàng thất bại',
								type: 'error',
							});
						}
					)
